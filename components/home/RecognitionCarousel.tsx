          ...
          <div className="flex touch-pan-y gap-[30px]">
            {recognitions.map((recognition, index) => (
              <div
                ...
                aria-label={`${index + 1} of ${recognitions.length}: ${recognition.name}`}
              >
                <Image
        ...
        <div className="mt-1 flex min-h-6 items-center justify-center" aria-label="Choose a recognition logo">
          {scrollSnaps.map((_, index) => (
            <button
              key={index}
              type="button"
              aria-label={`Show recognition set ${index + 1}`}
              aria-current={index === selectedIndex ? "true" : undefined}
              onClick={() => onDotClick(index)}
              className="group inline-flex size-6 items-center justify-center rounded-full"
            >
              <span aria-hidden="true" className={cn("size-[7px] rounded-full transition-colors", index === selectedIndex ? "bg-primary-deep" : "bg-black group-hover:bg-primary-bright")} />
            </button>
          ))}
        </div>
      </div>
